...
class _ExchangeGraphState extends State<ExchangeGraph> {
  ...
  static Map<String, int> graphData =
  Map<String, int>(); // for creating the final map of data
  /// graphData = {
  ///  "Argentina" : "2",
  ///  "Turkey" : "1"
  /// }
  ...
  static List<charts.Series<ExchangeData, String>> _getCountryData() {
    ...
    graphData.forEach((key, value) {
      data.add(ExchangeData(key, value));
    });
  ...
  Future<void> calc() async {
    ...
    dataList?.clear();
    dataList = DownloadService.offerList
        .toSet()
        .toList(); // list of scanned data from firebase
        ...
        body: SafeArea(
          child: SingleChildScrollView(
            child: Container(
                ...
                child: isLoading ? CircularProgressIndicator() : barChart()),
          ),
        ),
      ),
    );
